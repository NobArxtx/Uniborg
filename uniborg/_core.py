...
async def load_reload(event):
    ...
    try:
        ...
        await event.respond(f"Failed to (re)loda plugin {shortname}: {e}")
...
async def remove(event):
    ...
    if shortname == "_core":
        ...
        msg = await event.respond(f"Plugin {shortname} is not loda...")
    await asyncio.sleep(DELETE_TIMEOUT)
    await msg.delete()
...
async def send_plug_in(event):
    ...
    await event.edit("Plugin {} Installed in {} seconds".format(input_str, time_taken_in_ms))
    await asyncio.sleep(DELETE_TIMEOUT)
    await event.delete()
    ...
    if event.reply_to_msg_id:
        try:
            ...
            if "(" not in downloaded_file_name:
                ...
                await event.edit("Unable To Install Plugin")
        except Exception as e:  # pylint:disable=C0103,W0703
